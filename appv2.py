...
with st.expander("GUIA RAPIDA DE USO"):
    ...
    10. Si desea partir desde cero, use el botón **"Empezar de cero"**: se le pedirá confirmación y luego
        se borrarán los archivos y gráficos cargados en esta sesión (su historial de guardados NO se toca).
...
# ====================================
# REINICIAR APLICACIÓN ("Empezar de cero")
# ====================================
#
# A diferencia de "Borrar guardada" (que solo borra el autoguardado en disco),
# esto además limpia el estado en memoria (archivos, datasets, gráficos y
# selección) y fuerza que el widget de carga de archivos se reinicie (mediante
# el cambio de su "key"), para que un archivo recién quitado no reaparezca
# solo porque seguía técnicamente presente en el uploader. El historial de
# guardados manuales del usuario NO se borra con esta acción.
...
if "confirmar_reinicio" not in st.session_state:    # variables para "Empezar de cero"
    st.session_state.confirmar_reinicio = False
if "uploader_key" not in st.session_state:
    st.session_state.uploader_key = 0
if "archivo_a_quitar" not in st.session_state:      # variable para confirmar el borrado de un archivo puntual
...
def obtener_tamano_archivo(cantidad_bytes: int) -> str:
    """Formatea el tamaño de un archivo en KB o MB, con una etiqueta orientativa."""
...
def validar_calidad_datos(df: pd.DataFrame) -> list:
    ...
    if filas_duplicadas > 0:
        advertencias.append(f"Se encontraron {filas_duplicadas} fila(s) duplicada(s).")
...
# --- "Empezar de cero": borra archivos, datasets y gráficos de la sesión actual (pide confirmación) ---
if not st.session_state.confirmar_reinicio:
    if st.button("🧨 Empezar de cero"):
        st.session_state.confirmar_reinicio = True
        st.rerun()
else:
    st.warning("¿Está seguro de que desea borrar todos los archivos y gráficos cargados en esta sesión?")
    col_reinicio_si, col_reinicio_no = st.columns(2)
    with col_reinicio_si:
        if st.button("Sí, borrar todo"):
            empezar_de_cero()
            st.rerun()
    with col_reinicio_no:
...
for nombre, info in list(st.session_state.archivos_originales.items()):
    ...
    with st.expander(f"📄 {nombre}  ({info['tipo'].upper()}) - {tamano_archivo}", expanded=True):
        ...
        with col_quitar:
            if st.session_state.archivo_a_quitar == nombre:
                ...
                if st.button("Sí, quitar", key=f"confirmar_quitar_{nombre}"):
                    ...
                    st.session_state.archivo_a_quitar = None
                    # Cambia la key del uploader para que el archivo no se vuelva a agregar solo
                    st.session_state.uploader_key += 1
                    guardar_sesion()
                    st.rerun()
...
for nombre_ds, tab in zip(datasets_seleccionados, tabs):
    with tab:
        ...
        if columnas_numericas and not datos_filtrados.empty:
            ...
            minimo_kpi = datos_filtrados[columna_indicador].min()
            maximo_kpi = datos_filtrados[columna_indicador].max()
            mediana_kpi = datos_filtrados[columna_indicador].median()
            ...
            c4.metric("Mínimo", f"{minimo_kpi:,.2f}")
            c5.metric("Máximo", f"{maximo_kpi:,.2f}")
            c6.metric("Mediana", f"{mediana_kpi:,.2f}")

            kpis_dataset = {
                "Columna analizada": columna_indicador,
                "Total": f"{total:,.2f}",
                "Promedio": f"{promedio:,.2f}",
                "Mínimo": f"{minimo_kpi:,.2f}",
                "Máximo": f"{maximo_kpi:,.2f}",
                "Mediana": f"{mediana_kpi:,.2f}",
                "Registros": registros,
            }
        else:
